...
def test_mcp_auth_hotreloads_new_token_without_prior_request(data_dir):
    old_token = "old-token"
    new_token = "new-token"
    old_entry = TokenEntry(name="old", token_hash=hashlib.sha256(old_token.encode()).hexdigest())
    new_entry = TokenEntry(name="new", token_hash=hashlib.sha256(new_token.encode()).hexdigest())
    with make_client(data_dir, tokens=[old_entry]) as c:
        cs = ConfigService(data_dir)
        cs.load()
        cfg = cs.get()
        cfg.auth.tokens = [new_entry]
        cs.save_config(cfg)
        r = c.post("/mcp", headers={"Authorization": f"Bearer {new_token}"},
                   json={"jsonrpc": "2.0", "id": 1, "method": "initialize",
                         "params": {"protocolVersion": "2025-03-26",
                                    "capabilities": {}, "clientInfo": {"name": "t", "version": "1"}}})
        assert r.status_code == 200
